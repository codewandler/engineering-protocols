...
use ess_compiler::ir::{EssIr, ResolvedBody, ResolvedField, ResolvedInstance, ResolvedTypeRef};
...
impl SemanticDependencyGraph {
    ...
    /// each handle's [`name`](ess_compiler::ir::TypeHandle::name) and records an edge. No handle is carried into the
    ...
    /// because a [`ResolvedType`](ess_compiler::ir::ResolvedType) carries no `domain` field — the
        ...
        for (name, resolved) in &ir.commands {
            ...
            for outcome in &resolved.outcomes {
                ...
                if let Some(subject_of) = &outcome.subject {
                    ...
                    // Matched rather than read through `ResolvedInstance::event`, because this
                    // crate's source scan bans every `EssIr` handle accessor by spelling and that
                    // method shares one of their names.
