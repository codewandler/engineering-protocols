...
| **Four expectation kinds cannot be decided about a driven run, because the seam's wire does not carry what they read.** The `metaharness.event/1` adapter (`story:event-stream-trace-adapter`, 2026-08-22) lifts a driven transcript into `trace-ir/1` and reads every field the wire has; four kinds are `unk` against it and would be against any harness metaharness drives. `skill.completed` reads the vendor's `tool_use_result` sibling (`commandName`, `success`), which metaharness's Claude adapter does not carry — so the eval's strongest single claim is undecidable for a driven step, while `skill.invoked` still holds. `tokens.thinking`, `iterations` and `speed` have no key in the seam's `usage` payload, and a `cost.total` scoped to one model has none in its per-model record. Each reads `unk` and never a pass, which is correct and is not the same as being checkable | **not this repository's to close: it is four fields at the seam.** metaharness's event payloads are where the vendor's per-tool result sibling and the three usage figures would be carried; this reader gains them the day they arrive, with no rule changing here. The alternative — reading an absent `is_error` or an absent figure as *fine* the way the `stream-json` adapter reads an absent `is_error` — is refused by name: that adapter knows which vendor wrote its transcript and this one does not. Recorded with its reasoning in the story's *What a driven event stream cannot answer* table |
